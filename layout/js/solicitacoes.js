(function () {
  ...
  function buildCard(item) {
    ...
    card.className = `sol-card ${
      item.status === "Corrigida" ? "is-corrigida" : "is-pendente"
    }`;
    card.tabIndex = 0; // acessibilidade: permite foco no card
    card.innerHTML = `
      <div class="sol-head">
        <div class="aluno">
          <img class="aluno-avatar" src="${avatar(aluno)}" alt="${
      aluno.name || "Aluno"
    }">
        ...
        <div class="meta">
          <span class="sol-data"><i class="ri-calendar-line"></i> ${
            data || ""
          }</span>
          <span class="sol-status ${
            item.status === "Corrigida" ? "ok" : "pend"
          }">${item.status}</span>
        </div>
      </div>
      <div class="sol-body">
        ...
        <div class="texto">
          ${
            item.imagemUrl
              ...
              : `
                <span class="txt-preview">${
                  preview || "<i>Sem texto enviado</i>"
                }</span>
                ${
                  temResto
                    ? `<span class="txt-resto">${restante}</span>
      ...
      <div class="sol-actions">
        <button class="btn-corrigir" type="button" data-id="${
          item.id
        }" data-tipo="${item.tipoCorrecao || ""}">
    ...
    btnCorrigir?.addEventListener("click", () => {
      routeByType(
        btnCorrigir.getAttribute("data-tipo"),
        btnCorrigir.getAttribute("data-id")
      );
    });
  ...
  async function load() {
    ...
    try {
      ...
      if (!token) {
        ...
        $empty.textContent =
          "Faça login como corretor para ver as solicitações.";
        return;
      }

      const resp = await fetch(`${API}?status=pendente`, {
      ...
      const items = Array.isArray(data)
        ? data.filter(
            (item) => (item.status || "").toLowerCase() !== "corrigida"
          )
        : [];
