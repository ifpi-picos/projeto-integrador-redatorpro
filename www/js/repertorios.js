...
const repertorioCategorias = [
  { key: "cinema", title: "Cinema", icon: "ri-movie-2-line", description: "Filmes, documentarios e series" },
  { key: "livros", title: "Livros", icon: "ri-book-3-line", description: "Obras literarias e ensaios" },
  { key: "conhecimentos-gerais", title: "Conhecimentos Gerais", icon: "ri-earth-line", description: "Historia, filosofia, geografia e atualidades" },
  { key: "dados-pesquisas", title: "Dados e Pesquisas", icon: "ri-bar-chart-grouped-line", description: "Indicadores, estudos e levantamentos" },
  { key: "citacoes", title: "Citacoes", icon: "ri-double-quotes-l", description: "Frases de autores para contextualizar temas" },
];
...
const repertoriosFallback = [
  {
    ...
    streamingLinks: ["https://www.youtube.com/results?search_query=Tempos+Modernos"],
    trailerUrl: "https://www.youtube.com/results?search_query=Tempos+Modernos+trailer",
    synopsis: "Charlie Chaplin critica a mecanizacao do trabalho e a perda da dignidade humana em meio a rotina industrial.",
    essayUse: "Pode ser usado em temas sobre precarizacao do trabalho, alienacao produtiva e impactos sociais da tecnologia.",
  },
  {
    id: "seed-book-1",
    category: "livros",
    ...
    synopsis: "Carolina Maria de Jesus registra a rotina de fome, exclusao e resistencia na favela do Caninde.",
    essayUse: "Ajuda a discutir invisibilidade social, desigualdade estrutural e omissao do Estado.",
  },
  {
    id: "seed-knowledge-1",
    ...
    essayUse: "Serve para fundamentar argumentos sobre responsabilidade estatal, pacto coletivo e participacao cidada.",
  },
  {
    id: "seed-data-1",
    ...
    essayUse: "Use os dados para comprovar a gravidade do problema antes de apresentar causas e intervencoes.",
  },
  {
    id: "seed-quote-1",
    category: "citacoes",
    title: "A educacao e a arma mais poderosa que voce pode usar para mudar o mundo.",
    author: "Nelson Mandela",
    thematicAxes: ["Educacao", "Transformacao social", "Cidadania"],
    essayUse: "A citacao pode abrir repertorios sobre o papel da educacao na reducao de desigualdades.",
  },
];
...
function repertorioImage(item) {
  ...
  if (item.coverUrl && !String(item.coverUrl).toLowerCase().endsWith(".pdf")) {
...
function repertorioMeta(item) {
  if (item.category === "cinema") return [item.type, item.genre, item.duration, item.rating, item.country].filter(Boolean);
  if (item.category === "livros") return [item.genre, item.pages ? `${item.pages} paginas` : ""].filter(Boolean);
  if (item.category === "conhecimentos-gerais") return [item.knowledgeArea].filter(Boolean);
...
function createCard(item) {
  ...
  const quoteClass = item.category === "citacoes" ? " repertorio-card-quote" : "";
  return `
...
function destroyRepertorioSwipers() {
  repertoriosSwipers.forEach((swiper) => swiper && swiper.destroy && swiper.destroy(true, true));
  repertoriosSwipers = [];
}
...
function filterRepertorios(term) {
  ...
  renderRepertorios(
    repertoriosState.filter((item) =>
      [item.title, item.author, item.genre, item.knowledgeArea, item.info, item.synopsis, item.essayUse, normalizeList(item.thematicAxes).join(" ")]
        .join(" ")
        .toLowerCase()
        .includes(search),
    ),
...
async function fetchRepertorios() {
  try {
    const response = await fetch(`${REPERTORIOS_API_BASE}/repertorios`);
    if (!response.ok) throw new Error("Falha ao buscar repertorios");
    const data = await response.json();
    return Array.isArray(data) && data.length ? data : repertoriosFallback;
  } catch (error) {
    console.warn("Usando repertorios locais:", error);
...
window.initRepertorio = async function () {
  ...
  search?.addEventListener("input", (event) => filterRepertorios(event.target.value));
