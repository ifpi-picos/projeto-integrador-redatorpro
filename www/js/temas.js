...
let todosTemas = []; // Armazena todos os temas carregados

function carregarTemas() {
    fetch('https://express-e3hm.onrender.com/temas')
        .then(resp => resp.json())
        .then(temas => {
            todosTemas = temas; // Salva todos os temas para filtro posterior
...
window.filtrarTema = function(tipo) {
    if (!todosTemas.length) return;
    if (!tipo) {
        renderizarTemas(todosTemas);
    } else {
        renderizarTemas(todosTemas.filter(t => t.tipo && t.tipo.toLowerCase() === tipo.toLowerCase()));
    }
};
